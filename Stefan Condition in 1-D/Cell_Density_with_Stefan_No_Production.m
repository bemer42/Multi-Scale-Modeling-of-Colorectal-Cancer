%% This program will run the Cell Density PDE solver without the production
% term.  The very last section will create Figure 2 of the paper.  
close all;
clear all;
clc 
...
RHS_Q  = @(t,v,L) Chop(-s.*alpha./(L.^2).*(D_s(end,:)*Extend(v)).*(D_s*Extend(v)) + ...
                       (alpha./(L.^2)).*(D_s*((1./Extend(v).^2).*(D_s*Extend(v)))));
                       

RHS_L  = @(t,v,L) -alpha./L.*(D_s(end,:)*Extend(v));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
L_0 = 4;
q_0 = ones(size(s)) + (s*L_0<=2).*2.*ones(size(s));
...
for k = 1:20:N_t
   ...
   title('Cell Density with Stefan Condition','fontsize',30,'interpreter','latex')
   xlabel('crypt level ($s$)','fontsize',25,'interpreter','latex')
   ylabel('cell density ($q$)','fontsize',25,'interpreter','latex')
   set(gca,'fontsize',20)
   xlim([s_0 1.1*max(L)])
   ylim([0 1.1*max(max(Q))])
   grid on
   grid minor
   hold off
   if k == 1
...
%% Creating Figure 2 of Crypt Fission Paper:
% Four k values:
k1 = 1;
k2 = 6;
k3 = 40;
...
for i = 1:4
   ...
   set(gca,'fontsize',20)
   title('Cell Density with Stefan Condition','fontsize',35,'interpreter','latex')
   xlabel('crypt level ($\hat{s}$)','fontsize',25,'interpreter','latex')
   ylabel('cell density ($\hat{q}$)','fontsize',25,'interpreter','latex')
   text(.6,.8,['$\hat{\tau} = $ ' num2str(time) ' hrs'],'units','normalized','interpreter','latex','fontsize',20)
   text(.6,.65,['$\hat{L} = $ ' num2str(length)],'units','normalized','interpreter','latex','fontsize',20)
   xlim([s_0 1.1*max(L)])
   ylim([0 1.1*max(max(Q))])
   grid on
   grid minor
